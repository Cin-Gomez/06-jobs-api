//Calls a function, buildBooksTable. It populates the book table
    ...
    if (response.status === 200) {
      if (data.count === 0) {
        ...
        for (let i = 0; i < data.books.length; i++) {
          ...
          let rowHTML = `<td>${data.books[i].author}</td><td>${data.books[i].title}</td><td>${data.books[i].genre}</td><td>${data.books[i].status}</td>${editButton}${deleteButton}`;
  ...
  document.addEventListener("startDisplay", async () => {
    ...
    if (token) {
      ...
      if (count > 0) {
        ...
        booksMessage.textContent = "There are no books to display for this user.";
